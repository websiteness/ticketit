...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            ...
            <form method="POST" action="{{ route($setting->grab('admin_route').'.infinity.dev-status.store') }}">
                {{ csrf_field() }}
                                ...
                                <select class="select2 form-control" name="statuses[{{ $status->id }}]" style="width:100%;">
                                    ...
                                    @if($infinity_statuses)
                                        @foreach ($infinity_statuses as $infinity_status)
                                            @if(isset($status->infinity_item_id))
                                                <option value="{{ $infinity_status['id'] }}" {{$status->infinity_item_id == $infinity_status['id'] ? 'selected' : '' }}>{{ $infinity_status['name'] }}</option>                                
                                            @else
